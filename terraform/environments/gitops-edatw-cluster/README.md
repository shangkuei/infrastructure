...
This Terraform environment bootstraps Flux CD on the **edatw** Kubernetes cluster using the **Flux Operator** for GitOps-based continuous delivery.

> **Part 2 of 2**: This environment enables GitOps on an **existing running cluster** provisioned by [`talos-cluster-edatw-cluster/`](../talos-cluster-edatw-cluster/).
>
> **Prerequisites**: Complete [`talos-cluster-edatw-cluster/`](../talos-cluster-edatw-cluster/) deployment first. Cluster must be running and healthy.
...
Extract credentials from your edatw cluster:
...
```bash
# Add manifests to kubernetes/clusters/edatw/
git add kubernetes/
...
kubernetes/
└── clusters/
    └── edatw/                   # Cluster-specific configuration
...
|-----|---------|----------|
| Terraform | Encrypt terraform.tfvars, backend.hcl | `~/.config/sops/age/gitops-edatw-cluster.txt` |
| Flux | Encrypt Kubernetes manifests in Git | `~/.config/sops/age/gitops-edatw-cluster-flux.txt` |
...
- [Kubernetes manifests](../../../kubernetes/clusters/edatw/)
...
| <a name="input_cluster_name"></a> [cluster\_name](#input\_cluster\_name) | Name of the Kubernetes cluster | `string` | `"edatw"` | no |
| <a name="input_cluster_path"></a> [cluster\_path](#input\_cluster\_path) | Path in the repository where cluster manifests are stored | `string` | `"./kubernetes/clusters/edatw"` | no |
...
| <a name="input_sops_age_key_path"></a> [sops\_age\_key\_path](#input\_sops\_age\_key\_path) | Path to the SOPS age private key file for Flux decryption (deployed to Kubernetes) | `string` | `"~/.config/sops/age/gitops-edatw-cluster-flux.txt"` | no |
