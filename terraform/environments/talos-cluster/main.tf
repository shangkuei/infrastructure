# Talos Cluster Environment - Machine Configuration Generator
#
# This environment generates Talos machine configurations for a Kubernetes cluster
...
# 2. Review generated configs in ./generated/
...
# Generate custom Talos installer images with per-node extensions
# Create a map of unique extension combinations to avoid duplicate schematics
locals {
  # Collect all nodes with their extensions
  # Note: Node keys from variables are used as-is (e.g., "cp-01", "worker-01")
  all_nodes_with_extensions = merge(
    { for k, v in var.control_plane_nodes : k => v.extensions },
    { for k, v in var.worker_nodes : k => v.extensions }
  )

  # Create unique extension combinations (sorted for consistency)
  unique_extension_sets = toset([
    for node_key, extensions in local.all_nodes_with_extensions :
    join(",", sort(extensions))
  ])

  # Map each unique extension set to a schematic key
  extension_set_to_key = {
    for ext_set in local.unique_extension_sets :
    ext_set => replace(ext_set, ",", "-")
  }

  # Map each node to its schematic key (comma-separated extension list)
  node_to_schematic_key = {
    for node_key, extensions in local.all_nodes_with_extensions :
    node_key => join(",", sort(extensions))
  }
}

# Generate schematics for each unique extension combination
resource "talos_image_factory_schematic" "nodes" {
  for_each = local.extension_set_to_key
...
resource "local_file" "control_plane_config" {
  ...
  filename = "${path.module}/generated/control-plane-${each.key}.yaml"
...
resource "local_file" "control_plane_patches" {
  ...
  filename = "${path.module}/generated/control-plane-${each.key}-patch.yaml"
  content = yamlencode({
    machine = {
      network = {
...
resource "local_file" "control_plane_tailscale_extension" {
  ...
  filename = "${path.module}/generated/control-plane-${each.key}-tailscale.yaml"
  content = yamlencode({
    apiVersion = "v1alpha1"
...
resource "local_file" "worker_tailscale_extension" {
  ...
  filename = "${path.module}/generated/worker-${each.key}-tailscale.yaml"
  content = yamlencode({
    apiVersion = "v1alpha1"
    kind       = "ExtensionServiceConfig"
    name       = "tailscale"
...
resource "local_file" "cilium_values" {
  ...
  filename        = "${path.module}/generated/cilium-values.yaml"
