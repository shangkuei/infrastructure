...
output "cluster_name" {
  ...
  value       = module.gitops.cluster_name
}
...
output "deployment_phase" {
  ...
  value       = <<-EOT
    ╔════════════════════════════════════════════════════════════════════════════╗
    ║              SHANGKUEI-DEV GITOPS DEPLOYMENT GUIDE                          ║
    ╚════════════════════════════════════════════════════════════════════════════╝

    This configuration deploys Flux CD on the shangkuei-dev cluster.

    ══════════════════════════════════════════════════════════════════════════════
    Deployed Components
    ══════════════════════════════════════════════════════════════════════════════

    - cert-manager (namespace: ${module.gitops.cert_manager_namespace})
    - Flux Operator (namespace: ${module.gitops.flux_namespace})
    - FluxInstance + controllers (namespace: ${module.gitops.flux_namespace})
    ...
    kubectl -n ${module.gitops.flux_namespace} get fluxinstance flux

    # Check Git sync
    kubectl -n ${module.gitops.flux_namespace} get gitrepository
    kubectl -n ${module.gitops.flux_namespace} get kustomization

    Full documentation: terraform/environments/gitops-shangkuei-dev/README.md
  EOT
}
...
output "next_steps" {
  ...
  value       = <<-EOT
    ...
    2. Check FluxInstance status:
       kubectl -n ${module.gitops.flux_namespace} get fluxinstance flux -o jsonpath='{.status.conditions}' | jq

    3. Verify Git sync:
       kubectl -n ${module.gitops.flux_namespace} get gitrepository
       kubectl -n ${module.gitops.flux_namespace} get kustomization

    4. Add manifests to: ${module.gitops.cluster_path}
       - Push to branch: ${module.gitops.git_branch}
